...
class N2ElementFacebookAlbums extends N2ElementList
{
    ...
    function fetchElement() {
        ...
        N2JS::addInline('
            new NextendElementFacebookAlbums("' . $this->_id . '", "' . N2Base::getApplication('smartslider')->router->createAjaxUrl(array(
                "generator/getData",
                array(
        ...
        $api  = $info->getConfiguration()
                     ->getApi();
        try {
            $result = $api->api($this->_form->get('facebook-id', 'me') . '/albums');
