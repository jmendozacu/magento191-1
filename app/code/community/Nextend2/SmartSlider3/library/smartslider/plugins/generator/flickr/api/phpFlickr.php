    ...
    class phpFlickr
    {
        ...
        function getCached($request) {
            ...
            foreach ($request as $key => $value) {
                if (empty($value)) unset($request[$key]); else
        ...
        function request($command, $args = array(), $nocache = false) {
            ...
            if ($this->parsed_response['stat'] == 'fail') {
                if ($this->die_on_error) die("The Flickr API returned the following error: #{$this->parsed_response['code']} - {$this->parsed_response['message']}"); else {
            ...
            if (function_exists('curl_init')) {
                ...
                foreach ($rsp as $line) {
                    if (preg_match('|<err code="([0-9]+)" msg="(.*)"|', $line, $match)) {
                        if ($this->die_on_error) die("The Flickr API returned the following error: #{$match[1]} - {$match[2]}"); else {
        ...
        function get_auth_url($perms) {
            ...
            if ($this->service == "23") {
                ...
                return ("https://www.flickr.com/services/auth/?api_key=" . $this->api_key . "&perms=" . $perms . "&api_sig=" . $api_sig);
            }
        }
        ...
        function photosets_getList($user_id = NULL, $page = NULL, $per_page = NULL, $primary_photo_extras = NULL) {
            ...
            $this->request("flickr.photosets.getList", array(
                ...
                'page'                 => $page,
                'per_page'             => $per_page,
                'primary_photo_extras' => $primary_photo_extras
            ));
