    ...
    protected function _getData($count, $startIndex) {
        ...
        foreach ($photos AS $photo) {
            if (!isset($ownerCache[$photo['ownername']])) {
                ...
                $ownerCache[$photo['ownername']] = $client->people_getInfo($owner['nsid']);
            }
            ...
            $data[$i]['description'] = $photo['description'];
            $data[$i]['url']         = $ow['photosurl'];
            $data[$i]['url_label']   = n2_('View');

            $data[$i]['owner_username']       = $ow['username'];
            $data[$i]['author_name']          = $this->exists($ow['realname'], $ow['username']);
            $data[$i]['author_url']           = $ow['profileurl'];
