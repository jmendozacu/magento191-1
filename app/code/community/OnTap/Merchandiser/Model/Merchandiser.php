...
class OnTap_Merchandiser_Model_Merchandiser
{
    ...
    public function hightPosition($params)
    {
        $catId = $params['catId'];
        $merchandiserResourceModel = Mage::getResourceModel('merchandiser/merchandiser');
        $categoryProducts = $merchandiserResourceModel->getCategoryProduct($catId, "position DESC");
        $position = 1;
        foreach ($categoryProducts as $product) {
            $merchandiserResourceModel->updateProductPosition($catId, $product['product_id'], $position);
            $position++;
    ...
    public function lowestPosition($params)
    {
        $catId = $params['catId'];
        $merchandiserResourceModel = Mage::getResourceModel('merchandiser/merchandiser');
        $categoryProducts = $merchandiserResourceModel->getCategoryProduct($catId, "position ASC");
        $position = 1;
        foreach ($categoryProducts as $product) {
            $merchandiserResourceModel->updateProductPosition($catId, $product['product_id'], $position);
            $position++;
