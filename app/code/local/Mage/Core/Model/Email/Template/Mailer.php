...
class Mage_Core_Model_Email_Template_Mailer extends Varien_Object {
    ...
    public function setTemplateParams(array $templateParams) {
        ...
        if (Mage::getModel('newsletter/subscriber')->loadByEmail($email)->getSubscriberId() && Mage::getModel('newsletter/subscriber')->loadByEmail($email)->getSubscriberStatus() == 1) {
            $templateParams['customer']->setIsunsublink(1);
            $templateParams['customer']->setUnsubscriptionlink(Mage::getModel('newsletter/subscriber')->loadByEmail($email)->getUnsubscriptionLink());
        } else {
            $templateParams['customer']->setIsunsublink(0);
        }

        //print_r($templateParams['customer']);exit();
        /*
         * End
         */
