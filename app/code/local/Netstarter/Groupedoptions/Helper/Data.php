 ...
 */
class Netstarter_Groupedoptions_Helper_Data extends Mage_Core_Helper_Abstract
{
	/**
	 * Get product options
	 *
	...
	public function getProductViewJsonConfig($product) {
		...
		// must unregister to register the config product
		Mage::unregister('current_product');
		Mage::register('current_product', $product);
        $result =  Mage::getSingleton('groupedoptions/block_container_product_view')->setProduct($product)->getJsonConfig();
		Mage::unregister('current_product');
		Mage::register('current_product', $groupedProduct);
		return $result;
	}
    ...
    // Get lowestConfigPrice .
    // Input $product
    public function getLowestConfigPrice( $Arrproduct ) {
    	$childPriceLowest = '';
    	foreach ($Arrproduct as $product) {
    		$_product = Mage::getSingleton('catalog/product')->load( $product->getId() );
    		if ($childPriceLowest == '' || $childPriceLowest > $_product->getPrice() ) {
	            $childPriceLowest =  $_product->getPrice();
	        }
    	}
        return 'From: '.Mage::helper('core')->currency($childPriceLowest, true, false);
    }
}