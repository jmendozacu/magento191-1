 ...
 */
class Netstarter_Afeature_Block_Adminhtml_Widget_Chooser extends Netstarter_Afeature_Block_Adminhtml_Widget_Grid
{
    /**
    ...
    public function prepareElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        $element->setData('after_element_html', '<div><div>'.$hiddenHtml.'</div></div>'.$this->toHtml());
        return $element;
    }
    ...
    {
        $bannerValues = $this->getSelectedBanners() ? $this->getSelectedBanners() : '';
    ...
    {
        if ($column->getId() == 'bannername') {
            if ($column->getFilter()->getValue()) {
                $this->getCollection()->addFieldToFilter('title', array('like'=> '%'.$column->getFilter()->getValue().'%'));
            }
        }
        ...
        if ($column->getId() == 'in_banners') {
            ...
            if ($column->getFilter()->getValue()) {
                $this->getCollection()->getSelect()->where('afeature_id IN (?)',$bannerIds);
            } else {
                if ($bannerIds) {
                    $this->getCollection()->getSelect()->where('afeature_id NOT IN (?)',$bannerIds);
                }
            }
        }
    ...
    {
        return $this->getUrl('afeature/widget/grid', array('_current'=>true));
    }
