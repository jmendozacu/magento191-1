 ...
 */
class Netstarter_Faq_Block_Frontend_List extends Mage_Core_Block_Template
{
	...
	protected function _prepareLayout()
    {
        if ($head = $this->getLayout()->getBlock('head')) {
            $head->setTitle($this->__('Frequently Asked Questions') . ' - ' . $head->getTitle());
        }
    }
	 ...
	 *
	 * @param int $pageSize
	 * @return Netstarter_Faq_Model_Mysql_Faq_Collection collection of current FAQ entries
	 */
	public function getFaqCollection($pageSize = null)
	{
		if (!$this->_faqCollection || (intval($pageSize) > 0
			...
			$this->_faqCollection = Mage :: getModel('netstarter_faq/faq')
				->getCollection()
	...
	public function getCategoryCollection()
	{
	    ...
	    if (is_null($categories)) {
    	    $categories =  Mage::getResourceSingleton('netstarter_faq/category_collection')
    	       ->addStoreFilter(Mage::app()->getStore())
    	       ->addIsActiveFilter();
	 ...
	 * 
	 * @param Netstarter_Faq_Model_Category $category
	 * @return Netstarter_Faq_Model_Mysql4_Faq_Collection
	 */
	public function getItemCollectionByCategory(Netstarter_Faq_Model_Category $category)
	{
	...
	public function getFaqJumplist()
	{
		...
		{
			$this->_faqJumplist = Mage::helper('netstarter_faq/jumplist');
    ...
    /*
     * Format the record into html
     */
    public function convertItemsToHtml($item){
        $processor = Mage::getModel('core/email_template_filter');
        $content = $processor->filter($item);
        return $content;
    }
}
