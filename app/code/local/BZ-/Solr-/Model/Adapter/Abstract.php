 ...
 */
abstract class BZ_Solr_Model_Adapter_Abstract
{
    /**
     ...
     * @return BZ_Solr_Model_Adapter_Abstract
     */
    ...
    protected function _prepareProductCategoryIndexData($productId, $storeId)
    {
        ...
        $categoryProductData = Mage::getResourceSingleton('bz_solr/index')
    ...
    {
        $result = array();
        $productPriceIndexData = Mage::getResourceSingleton('bz_solr/index')
    ...
    protected function _prepareIndexProductData($productIndexData, $productId, $storeId)
    {
        ...
        //unique will be id|store_id|type
        $productIndexData[self::UNIQUE_KEY] = $productId . '|' . $storeId . '|' . 'product';

        return $productIndexData;
    }
    
    public function prepareDocsPerStore($docData, $storeId, $entityType = 'product')
    {
        ...
        foreach ($docData as $productId => $productIndexData) {
            ...
            $display_name = $productIndexData['name'];
            $productIndexData = $this->_prepareIndexProductData($productIndexData, $productId, $storeId, $entityType);
            if (!$productIndexData) {
                continue;
            }
            //add display name for group and configurable products
            if (is_array($display_name)) {
                $array_values = array_values($display_name);
                $productIndexData['display_name_en'] = array_shift($array_values);
            }
            else $productIndexData['display_name_en'] = $display_name;
            
            //indexing the product images and url
            //Mage::getSingleton('core/design_package')->setArea('frontend');
            //Mage::app()->getStore()->setStoreId($storeId);
            $image = Mage::getResourceModel('catalog/product')->getAttributeRawValue($productId, 'image', $storeId);
            $thumbnail = Mage::getResourceModel('catalog/product')->getAttributeRawValue($productId, 'thumbnail', $storeId);
            $small_image = Mage::getResourceModel('catalog/product')->getAttributeRawValue($productId, 'small_image', $storeId);
            $url_key = Mage::getResourceModel('catalog/product')->getAttributeRawValue($productId, 'url_key', $storeId);
            /* create cache image in loading not in the indexing as it is too slow
            $cached_thumbnail = Mage::getModel('catalog/product_image')
                            ...
                            ->saveFile()
                            ->getUrl();*/
            $productIndexData['url_key_en'] = $url_key;
            $productIndexData['image_en'] = $image;
            $productIndexData['small_image_en'] = $small_image;
            $productIndexData['thumbnail_en'] = $thumbnail;
            foreach ($productIndexData as $name => $value) {
                if (is_array($value)) {
                    foreach ($value as $val) {
                        //adding check here check value not empty to be index
     ...
     * @return array
     */
    public function getIdsByQuery($query, $params = array(), $producType = 'product')
    {
    ...
    protected function _prepareQueryResponse($response)
    {
        ...
        foreach ($_docs as $doc) {
            $arr = $this->_objectToArray($doc);
            //separate result by entities
            if(isset($arr['type']) && $arr['type'] == 'product'){
                $result['product'][] = $arr;
            }
        }
        return $result;
    ...
    {
        $words = explode(' ', $text);
        if (isset($words[0]) && count($words[0]) > 1) {
            foreach ($words[0] as $key => &$val) {
                if (!empty($val)) {
