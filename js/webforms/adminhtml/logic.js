...
function Admin_JsWebFormsLogicTargetCheck(target, logicRules, prefix) {
    ...
    if ($(target["id"] + '_container')!== null && typeof($(target["id"] + '_container')) == 'object' && $(target["id"] + '_container').style) {
        ...
        if(display == 'none'){
            $(target["id"] + '_container').getElementsBySelector('.required-entry').each(function(s,i){s.disable();});
        } else {
            $(target["id"] + '_container').getElementsBySelector('.required-entry').each(function(s,i){s.enable();});
        }
    }
