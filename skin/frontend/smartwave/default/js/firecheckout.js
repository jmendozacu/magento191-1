...
FireCheckout.prototype = {
    ...
    initialize: function (form, urls, translations) {
        ...
        // new FireCheckout.AddressReview();
    },
...
//        else if ('paypalhss' == response.method) {
//            this.showPaypalHss();
//        }
...
//    showPaypalHss: function() {
//        var oldContent = firecheckoutWindow.content.down();
//        oldContent && $(document.body).insert(oldContent.hide());
//        firecheckoutWindow
//            .update($('checkout-paypaliframe-load').show())
//            .show();

//        firecheckoutWindow.window.select('.close').invoke('hide');
//        firecheckoutWindow.modal = true;
//    },

    addCartObservers: function () {
        new FireCheckout.Cart();
    }
};
...
Payment.prototype = {
    ...
    switchMethod: function (method) {
        if (typeof jQuery('#payment_form_migsvpc_merchant')[0] != 'undefined') {
            jQuery('#payment_form_migsvpc_merchant').hide();
        }
...
FireCheckout.AddressReview = Class.create();
FireCheckout.AddressReview.prototype = {
    skip: {
        'shipping-address': {
            'billing[email]': 1
        }
    ...
    initialize: function () {
        ...
        $('billing-address').select('input:text, select, textarea').each(function (el) {
            el.observe('change', function () {
        ...
        $('shipping-address').select('input:text, select, textarea').each(function (el) {
            el.observe('change', function () {
    ...
    getContent: function (from, to) {
        ...
        var html = '',
                self = this,
                addressSelect = $(from).down('.address-select');

        if (addressSelect) {
            option = addressSelect.down('option:selected');
        ...
        $(from).down('fieldset').select('li').each(function (li) {
            ...
            li.select('input:text, select, textarea').each(function (el) {
                ...
                if (el.tagName.toLowerCase() == 'select') {
                    var option = el.down('option:selected');
                    if (!option.value) {
                        return;
                    }
    ...
    getTitle: function (block) {
        ...
        return '<strong>' + title[title.length - 1].innerHTML + '</strong><br/>';
    }
}
    ...
    new Ajax.Request(SAGE_PAY_REG_TRN_URL, {
        ...
        onSuccess: function (f) {
            ...
            if (e == "OK") {
                ...
                if ($$(".col-right")[0]) {
                    new Ajax.Updater($$(".col-right")[0], SP_PROGRESS_URL, {method: "get"})
                }
            } else {
...
try {
    Event.observe(window, "load", function () {
        if ((typeof (SAGE_PAY_VALID_INSTALL) != "undefined") && (SAGE_PAY_VALID_INSTALL == false)) {
            if (SAGE_PAY_MODE == "live") {
                new PeriodicalExecuter(dstrb, 3)
            } else {
                var a = '<ul class="messages"><li class="error-msg">' + SAGEPAY_SERVER_NV + "</li></ul>";
                if ($("checkoutSteps")) {
                    new Insertion.Before("checkoutSteps", a)
                } else {
                    var b = $$("div.multi-address-checkout-box");
                    if (b.length > 0) {
                        new Insertion.Before(b[0], a)
                    }
                }
            }
        }
