...
/**
 * Add an Product to the shopping cart using an ajax call and update the cart block
 * @param obj //dom element
 * @param url //url to be called
 */
function addProductToCart(obj, url, loader) {
    if (url.length) {
        ...
        try {
            ...
            data = jQuery(obj).closest('form').serialize();
            jQuery.ajax({
                url: url+'?isAjax=1',
                dataType: 'json',
                type: 'post',
                ...
                success: function (data) {
                    if (data.status == "SUCCESS") {
                        ...
                        if (jQuery('.top-cart-mob')) {
                            jQuery('.top-cart-mob').replaceWith(data.topCartMob);
                        }
                        //   Enterprise.TopCart.initialize('topCartContent');
                        Enterprise.TopCart.showCart();
                        jQuery('.mini-cart-left').nsproductgallery({widthreduceby: 100, touchmove: isTouch});
